...
export default function CategoryPage({params}){const category=categories.find(x=>x.slug===params.categoryId);if(!category)notFound();const posts=getCategoryArticles(category.slug);return <main className="bh-wrap"><section className="bh-section"><p className="bh-eyebrow">The blog · {category.titleShort||category.title}</p><h1 className="bh-title">{category.title}</h1><p className="bh-lead">{category.description}</p><Link className="bh-text-link" href="/blog">Explore all guides →</Link></section><section className="bh-project-grid" style={{paddingBottom:80}}>{posts.map(x=><CardArticle key={x.slug} article={x}/>)}</section></main>;}
