...
export default function CreateTemplateServiceModal({ onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        const newTemplateService = await response.json();
        onSuccess(newTemplateService);
      } else {
        const errorData = await response.json();
        console.error("Failed to create template service:", errorData);
        alert(
          `Failed to create template service: ${errorData.error || "Unknown error"}`,
        );
      }
    } catch (error) {
      console.error("Error creating template service:", error);
      alert(
        "Error creating template service. Please check the console for details.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className="p-6">
            ...
            <div className="mt-6 flex justify-end space-x-3">
              ...
              >
                ...
                {loading ? "Creating..." : "Create Template Service"}
              </button>
            </div>
          </form>
        </div>
