...
export default function EditRateCardModal({ rateCard, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (rateCard) {
      setFormData({
        name: rateCard.name || "",
        type: rateCard.type || "tradesperson",
        category: rateCard.category || "",
        unit: rateCard.unit || "",
        price: rateCard.price || "",
        description: rateCard.description || "",
        isActive: rateCard.isActive !== undefined ? rateCard.isActive : true,
      });
    }
  }, [rateCard]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`/api/admin/quoting/rates/${rateCard._id}`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        console.error("Failed to update rate card");
      }
    } catch (error) {
