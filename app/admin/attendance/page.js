...
export default function AttendanceAdminPage() {
    ...
    const load = async () => {
      try {
        const [wRes, pRes] = await Promise.all([
          fetch("/api/workers?active=true").then((r) => r.json()),
          fetch("/api/projects?status=On Going").then((r) => r.json()),
        ]);
        setWorkers(wRes.workers || []);
        const mappedProjects = (pRes.projects || []).map((p) => ({
          id: normalizeId(p),
          name: p.name,
        }));
  ...
  // Fetch attendance for current visible month
  useEffect(() => {
    ...
    // Calendar events (no pagination)
    fetch(`/api/attendance?${params.toString()}`)
      .then((r) => r.json())
      .then((data) => {
        const evts = (data.items || []).map((a, index) => ({
  ...
  }, [date, selectedWorkerId, selectedProjectId, listPage, listLimit]);
  ...
  async function confirmDeleteAttendance() {
    ...
    try {
      ...
      if (res.ok) {
        // Force refresh by creating a new date object with current timestamp
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const res = await fetch(`/api/workers?query=${encodeURIComponent(qs)}`);
  ...
  useEffect(() => {
    const fetchDrawer = async () => {
      if (!isDrawerOpen || !drawerWorker?._id) return;
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="mb-2 text-3xl font-bold text-gray-900">Attendance</h1>
          ...
          {mounted && localizer ? (
            <div className="attendance-calendar">
              <Calendar
                ...
                onSelectEvent={(evt) => openWorkerDrawer(evt.resource?.worker)}
                selectable
                style={{ height: 600 }}
          ...
          <div className="mb-4 flex items-center gap-4">
            <label>
              ...
              <input
                ...
                value={listPage}
                onChange={(e) =>
                  setListPage(Math.max(1, parseInt(e.target.value || "1", 10)))
                }
            ...
            <div className="text-sm text-gray-600">Total: {listTotal}</div>
          </div>
                  ...
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      >
                        <div className="relative">
                          <Combobox.Input
                            ...
                            onChange={(e) => setAddWorkerQuery(e.target.value)}
                      ...
                      <div className="mt-2 text-xs text-gray-500">
                        Missing a person? Create one in Workers and try again.
                      </div>
                    </div>
                      ...
                      >
                        <div className="relative">
                          <Combobox.Input
                            ...
                            onChange={(e) => setAddProjectQuery(e.target.value)}
                          ...
                          <Combobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded border bg-white py-1 shadow">
                            {filteredProjects.map((p) => (
                              <Combobox.Option
                                key={p.id}
                                value={
                                  p.isCustom ? p : { id: p.id, name: p.name }
                                }
                                className={({ active }) =>
                  ...
                  <div className="mt-6 flex justify-end gap-2">
                    ...
                    <button
                      className={`rounded px-3 py-1 text-white ${
                        isSaving || !addSelectedWorker || !addSelectedProject
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <label className="text-sm sm:col-span-2">
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setNewWorker({ ...newWorker, notes: e.target.value })
                        }
                      />
                    </label>
                  </div>
      ...
      <Transition.Root show={isDrawerOpen} as={Fragment}>
        <Dialog as="div" className="relative z-40" onClose={setIsDrawerOpen}>
          <Transition.Child
            as={Fragment}
            enter="ease-out duration-300"
            enterFrom="opacity-0"
                        ...
                        <div className="mb-4 flex flex-wrap items-end gap-3">
                          ...
                          <label className="text-sm">
                            ...
                            <select
                              ...
                              value={drawerLimit}
                              onChange={(e) => {
                                setDrawerLimit(parseInt(e.target.value, 10));
                                setDrawerPage(1);
                              }}
                            >
                              {[10, 25, 50, 100].map((n) => (
