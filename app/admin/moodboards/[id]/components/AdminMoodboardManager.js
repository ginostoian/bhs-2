...
/**
 * Admin Moodboard Manager Component
 * Manages a specific moodboard with sections and products
 */
export default function AdminMoodboardManager({
  ...
  const handleProductSubmit = async (productData) => {
    ...
    try {
      ...
      // Update sections with new product
      setSections((prev) =>
        prev.map((section) =>
  ...
  // Handle admin comment update
  const handleAdminCommentUpdate = async (
    sectionId,
    productId,
    adminComment,
  ) => {
    try {
      ...
      setModalState({
        isOpen: true,
        title: "Error",
        message: "Failed to update admin comment. Please try again.",
        type: "alert",
        confirmText: "OK",
      });
    }
      ...
      <div className="space-y-6">
        {sections.map((section) => (
          <SectionManager
            ...
            onAdminCommentUpdate={(productId, adminComment) =>
              handleAdminCommentUpdate(section.id, productId, adminComment)
            }
            onAddProduct={() =>
