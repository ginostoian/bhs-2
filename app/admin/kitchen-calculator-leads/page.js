...
export const metadata = {
  title: "Kitchen Calculator Leads - Admin",
  description: "View leads captured through the kitchen cost calculator",
};
...
export default async function KitchenCalculatorLeadsPage() {
  await connectMongoose();

  const leads = await Lead.find({
    source: "Other",
    customSource: "Kitchen Calculator",
    isActive: true,
    isArchived: false,
  })
    ...
    <div className="space-y-6 pb-10">
      <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-600">
          Leads captured from the kitchen calculator, including the latest
          estimate, scope inputs, and PDF email delivery status.
        </p>
      </div>
            ...
            <tbody className="divide-y divide-gray-50 bg-white">
                  ...
                  <tr key={String(row._id)} className="align-top hover:bg-gray-50/70">
                    ...
                    <Cell>
                      <div className="text-sm text-gray-900">
                        {pretty(row.input?.kitchenRange)} • {pretty(row.input?.appliancePackage)}
                      </div>
                      <div className="mt-1 text-xs text-gray-500">
                        Worktop: {pretty(row.input?.worktopType)} • Splashback:{" "}
                        {pretty(row.input?.splashbackType)}
                      </div>
                      <div className="mt-1 text-xs text-gray-500">
                        Electrical: {pretty(row.input?.electricalLevel)} • Plumbing:{" "}
                        {pretty(row.input?.plumbingLevel)}
                      </div>
                    </Cell>
              ...
              {rows.length === 0 && (
                <tr>
                  ...
                  >
                    No kitchen calculator leads found yet.
                  </td>
                </tr>
              )}
            </tbody>
