...
export default function PDFDownload({ calculationResult, formData }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await publicFormFetch("/api/leads", {
        method: "POST",
      ...
      // Download a local copy immediately for a smooth UX.
      downloadPDF(data?.estimate || calculationResult, formData, email.trim());
      setIsSubmitted(true);
    } catch (submitError) {
      console.error("Lead capture error:", submitError);
      ...
      <div className="grid gap-5 lg:grid-cols-[1.1fr_1fr] lg:items-start">
        <div>
          ...
          <p className="mt-2 text-sm text-stone-600">
            We&apos;ll save your estimate and email a PDF with the cost
          ...
          <div className="mt-4 rounded-xl border border-stone-200 bg-stone-50 p-4">
            ...
            <ul className="mt-2 space-y-1 text-sm text-stone-700">
              ...
              <li>Line-item breakdown (build, fees, contingency, VAT)</li>
              <li>Assumptions and exclusions checklist</li>
              <li>Best-practice next steps before requesting quotes</li>
            </ul>
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-start gap-2 rounded-xl border border-stone-200 bg-white px-3 py-2.5">
              ...
              <span className="text-xs leading-relaxed text-stone-700">
                I agree to receive this estimate by email and understand this is
                an early budgeting tool, not a final quote.
              </span>
            </label>
