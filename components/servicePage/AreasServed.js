  ...
  return (
    <section id={id} className="scroll-mt-24 mx-auto max-w-[88%] px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
      <div className="mb-10 max-w-3xl">
        <p className="mb-3 text-sm font-extrabold uppercase tracking-[0.24em] text-[#266bf1]">Areas we serve</p>
        <h2 className="text-3xl font-black leading-tight text-[#100b47] md:text-5xl">{title}</h2>
      ...
      <div className="grid gap-5 lg:grid-cols-3">
        {groups.map((group) => (
          <article key={group.regionTitle} className="rounded-2xl border border-[#d9e5fb] bg-white p-6 shadow-sm">
            <h3 className="text-xl font-black text-[#100b47]">{group.regionTitle}</h3>
            ...
            <ul className="mt-5 grid grid-cols-2 gap-x-4 gap-y-1">
              {group.locations.map((location) => (
                <li key={location.slug} className="border-t border-[#edf2fc] py-3">
                  <Link href={`/locations/${location.slug}`} className="text-sm font-bold text-[#266bf1] hover:text-[#1449B0] hover:underline">
                    {location.name}
                  </Link>
                </li>
              ))}
